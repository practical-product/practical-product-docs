...
<Mermaid chart={`
erDiagram
  CATEGORY["CATEGORY (카테고리)"] ||--o{ PRODUCT["PRODUCT (상품)"] : "분류한다"
  PRODUCT["PRODUCT (상품)"] ||--o{ PRODUCT_OPTION_GROUP["PRODUCT_OPTION_GROUP (상품 옵션 그룹)"] : "가진다"
  PRODUCT_OPTION_GROUP["PRODUCT_OPTION_GROUP (상품 옵션 그룹)"] ||--o{ PRODUCT_OPTION["PRODUCT_OPTION (상품 옵션)"] : "가진다"
  PRODUCT["PRODUCT (상품)"] ||--o{ PRODUCT_SKU["PRODUCT_SKU (상품 SKU)"] : "가진다"
  PRODUCT_SKU["PRODUCT_SKU (상품 SKU)"] ||--|| PRODUCT_STOCK["PRODUCT_STOCK (상품 재고)"] : "재고를 가진다"
  PRODUCT["PRODUCT (상품)"] ||--o{ PRODUCT_IMAGE["PRODUCT_IMAGE (상품 이미지)"] : "가진다"
  MEMBER["MEMBER (회원)"] ||--o{ ORDERS["ORDERS (주문)"] : "주문한다"
  MEMBER["MEMBER (회원)"] ||--o{ REFRESH_TOKEN["REFRESH_TOKEN (리프레시 토큰)"] : "발급받는다"
  ORDERS["ORDERS (주문)"] ||--o{ ORDER_ITEM["ORDER_ITEM (주문 아이템)"] : "포함한다"
  ORDERS["ORDERS (주문)"] ||--o| PAYMENT["PAYMENT (결제)"] : "결제된다"
  ORDERS["ORDERS (주문)"] ||--o{ STOCK_RESERVATION["STOCK_RESERVATION (재고 예약)"] : "재고를 예약한다"
  PRODUCT_SKU["PRODUCT_SKU (상품 SKU)"] ||--o{ ORDER_ITEM["ORDER_ITEM (주문 아이템)"] : "주문된다"
  PRODUCT_SKU["PRODUCT_SKU (상품 SKU)"] ||--o{ STOCK_RESERVATION["STOCK_RESERVATION (재고 예약)"] : "예약된다"
  ...
  PRODUCT_OPTION_GROUP["PRODUCT_OPTION_GROUP (상품 옵션 그룹)"] {
    bigint id PK
    bigint product_id
    varchar name
  }
  PRODUCT_OPTION["PRODUCT_OPTION (상품 옵션)"] {
    bigint id PK
    bigint option_group_id
    varchar name
    bigint additional_price
  }
  PRODUCT_SKU["PRODUCT_SKU (상품 SKU)"] {
    bigint id PK
    bigint product_id
    varchar code UK
    varchar option_name
  ...
  PRODUCT_STOCK["PRODUCT_STOCK (상품 재고)"] {
    bigint id PK
    bigint sku_id UK
    int quantity
  }
  PRODUCT_IMAGE["PRODUCT_IMAGE (상품 이미지)"] {
    bigint id PK
    bigint product_id
    varchar image_url
    int sort_order
  ...
  STOCK_RESERVATION["STOCK_RESERVATION (재고 예약)"] {
    bigint id PK
    bigint sku_id
    bigint order_id
    int quantity
  ...
  OUTBOX_EVENT["OUTBOX_EVENT (아웃박스 이벤트)"] {
    bigint id PK
    varchar event_type
    json payload
    varchar status
