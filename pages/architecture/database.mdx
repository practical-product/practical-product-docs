...
erDiagram
  ...
  "CATEGORY (카테고리)" {
    bigint "카테고리ID(id)" PK
    varchar "카테고리코드(code)" UK
    varchar "카테고리명(name)"
    bigint "부모카테고리ID(parent_id)"
    boolean "노출여부(is_displayed)"
  }
  "PRODUCT (상품)" {
    bigint "상품ID(id)" PK
    varchar "상품코드(code)" UK
    bigint "카테고리ID(category_id)"
    varchar "상품명(name)"
    varchar "상품설명(description)"
    bigint "가격(price)"
    varchar "상품상태(status)"
    boolean "삭제여부(is_deleted)"
  }
  "PRODUCT_OPTION_GROUP (옵션 그룹)" {
    bigint "옵션그룹ID(id)" PK
    bigint "상품ID(product_id)"
    varchar "그룹명(name)"
    int "표시순서(display_order)"
  }
  "PRODUCT_OPTION (옵션)" {
    bigint "옵션ID(id)" PK
    bigint "옵션그룹ID(option_group_id)"
    varchar "옵션명(name)"
    bigint "추가금(additional_price)"
  }
  "PRODUCT_SKU (SKU)" {
    bigint "SKU_ID(id)" PK
    bigint "상품ID(product_id)"
    varchar "SKU코드(code)" UK
    varchar "옵션조합명(option_name)"
    bigint "판매가(price)"
  }
  "PRODUCT_STOCK (재고)" {
    bigint "재고ID(id)" PK
    bigint "SKU_ID(sku_id)" UK
    int "재고수량(quantity)"
  }
  "PRODUCT_IMAGE (이미지)" {
    bigint "이미지ID(id)" PK
    bigint "상품ID(product_id)"
    varchar "이미지URL(image_url)"
    int "정렬순서(sort_order)"
  }
  "MEMBER (회원)" {
    bigint "회원ID(id)" PK
    varchar "이메일(email)" UK
    varchar "이름(name)"
    varchar "닉네임(nickname)"
    varchar "회원상태(status)"
  }
  "ORDERS (주문)" {
    bigint "주문ID(id)" PK
    varchar "주문번호(order_no)" UK
    bigint "회원ID(member_id)"
    varchar "주문상태(status)"
    bigint "총액(total_price)"
    datetime "결제완료일시(paid_at)"
  }
  "ORDER_ITEM (주문 아이템)" {
    bigint "주문아이템ID(id)" PK
    bigint "주문ID(order_id)"
    bigint "상품ID(product_id)"
  ...
  "PAYMENT (결제)" {
    bigint "결제ID(id)" PK
    bigint "주문ID(order_id)" UK
    varchar "결제키(payment_key)" UK
    bigint "결제금액(amount)"
    varchar "결제상태(status)"
  }
  "STOCK_RESERVATION (재고 예약)" {
    ...
    int "예약수량(quantity)"
    varchar "예약상태(status)"
    datetime "만료시각(expires_at)"
  }
  "OUTBOX_EVENT (아웃박스 이벤트)" {
    bigint "이벤트ID(id)" PK
    varchar "이벤트타입(event_type)"
    json "페이로드(payload)"
    varchar "발행상태(status)"
  }
  "REFRESH_TOKEN (리프레시 토큰)" {
    bigint "토큰ID(id)" PK
    bigint "회원ID(member_id)"
    varchar "토큰체인ID(token_family)"
    varchar "토큰식별자(jti)" UK
    varchar "토큰상태(status)"
  }
`} />
