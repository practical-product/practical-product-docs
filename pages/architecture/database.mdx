...
erDiagram
  ...
  "CATEGORY (카테고리)" {
    ...
    varchar code UK "카테고리 코드"
    varchar name "카테고리명"
    bigint parent_id "부모 카테고리 ID"
    boolean is_displayed "노출 여부"
  }
  "PRODUCT (상품)" {
    bigint id PK "상품 ID"
  ...
  "PRODUCT_OPTION (옵션)" {
    bigint id PK "옵션 ID"
    bigint option_group_id "옵션그룹 ID"
    varchar name "옵션명"
  ...
  "PRODUCT_SKU (SKU)" {
    bigint id PK "SKU ID"
    bigint product_id "상품 ID"
    varchar code UK "SKU 코드"
    varchar option_name "옵션조합명"
    bigint price "판매가"
  }
  "PRODUCT_STOCK (재고)" {
    bigint id PK "재고 ID"
    bigint sku_id UK "SKU ID"
    int quantity "재고수량"
  }
  "PRODUCT_IMAGE (이미지)" {
    bigint id PK "이미지 ID"
  ...
  "ORDERS (주문)" {
    ...
    varchar status "주문상태"
    bigint total_price "총액"
    datetime paid_at "결제완료일시"
  }
  "ORDER_ITEM (주문 아이템)" {
    bigint id PK "주문아이템 ID"
  ...
  "STOCK_RESERVATION (재고 예약)" {
    ...
    int quantity "예약수량"
    varchar status "예약상태"
    datetime expires_at "만료시각"
  }
  "OUTBOX_EVENT (아웃박스 이벤트)" {
    bigint id PK "이벤트 ID"
    varchar event_type "이벤트타입"
    json payload "페이로드"
    varchar status "발행상태"
  }
  "REFRESH_TOKEN (리프레시 토큰)" {
    bigint id PK "토큰 ID"
    bigint member_id "회원 ID"
    varchar token_family "토큰체인ID"
    varchar jti UK "토큰식별자"
    varchar status "토큰상태"
  }
`} />
